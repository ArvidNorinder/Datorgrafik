...
{
	// Calculate the number of vertices for latitude and longitude slices
	auto const lat_slice_vertices_count = latitude_split_count + 1u;
	auto const lon_slice_vertices_count = longitude_split_count + 1u;
	auto const vertices_nb = lat_slice_vertices_count * lon_slice_vertices_count; //TODO

	// Initialize vertex attributes (position, normal, texture coordinates, tangents, binormals)
	auto vertices = std::vector<glm::vec3>(vertices_nb);
	auto normals = std::vector<glm::vec3>(vertices_nb);
	auto texcoords = std::vector<glm::vec3>(vertices_nb);
	auto tangents = std::vector<glm::vec3>(vertices_nb);
	...
	// Angle increments for latitude (phi) and longitude (theta)
	float const d_phi = glm::pi<float>() / static_cast<float>(latitude_split_count);     // Polar angle step (latitude)
	float const d_theta = glm::two_pi<float>() / static_cast<float>(longitude_split_count); // Azimuthal angle step (longitude)

	// Generate vertices
	size_t index = 0u;
	for (unsigned int i = 0u; i < lat_slice_vertices_count; ++i) {
		float const phi = static_cast<float>(i) * d_phi;
		...
		for (unsigned int j = 0u; j < lon_slice_vertices_count; ++j) {
			...
			auto const b = glm::vec3(radius * sin_theta * cos_phi,
				radius * sin_phi,
				radius * cos_theta * cos_phi);  // Binormal vector
	...
	// Create index array for the sphere's triangles
	auto index_sets = std::vector<glm::uvec3>(2u * latitude_split_count * longitude_split_count);

	// Generate indices iteratively
	index = 0u;
	for (unsigned int i = 0u; i < latitude_split_count; ++i) {
		for (unsigned int j = 0u; j < longitude_split_count; ++j) {
	...
	auto const longitude_vertices_count = longitude_split_count + 1u;
	auto const vertices_nb = latitude_vertices_count * longitude_vertices_count;
	...
	float const d_angle = glm::two_pi<float>() / (static_cast<float>(latitude_vertices_count));
	...
	for (unsigned int i = 0u; i < longitude_vertices_count; ++i) {
		...
		float const cos_theta = std::cos(phi);
		float const sin_theta = std::sin(phi);
	...
	for (unsigned int i = 0u; i < latitude_split_count; ++i)
	{
		for (unsigned int j = 0u; j < longitude_split_count; ++j)
		{
			...
			index_sets[index] = glm::uvec3(
				longitude_vertices_count * i + j,
				longitude_vertices_count * i + (j + 1) % longitude_vertices_count,
				longitude_vertices_count * (i + 1) + (j + 1) % longitude_vertices_count
			);
			++index;
			...
			index_sets[index] = glm::uvec3(
				longitude_vertices_count * i + j,
				longitude_vertices_count * (i + 1) + (j + 1) % longitude_vertices_count,
				longitude_vertices_count * (i + 1) + j
			);
			++index;
		}
	}
